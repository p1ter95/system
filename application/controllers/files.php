...
class Files extends CI_Controller {
		...
		if($this->system_model->isLogged())
		{
			...
			if(file_exists($dest . '/' . $basename) && $this->input->post('copy_file_overwrite') === 'false') {
            	...
            	{     		
            		...
            		{
            			if(copy($path, $dest . '/' . $str . $filename . ' [' . $i . '].' . $extension))
            			{
            				$data = TRUE;
            			}
            			else
	...
	public function get_filenames()
	{
		...
		if($this->system_model->isLogged())
		{
			...
			if(!$output = get_filenames($path, ($this->input->post('get_filenames_include_path') == 'true' ? true : false)))
			{
				$output = FALSE;
			}
    	}
	...
	public function create_dir()
	{
		...
		if($this->system_model->isLogged())
		{
	    	...
	    	if(@mkdir($path, octdec($this->input->post('create_dir_mode')), TRUE)) 
	    	{	
	      		$output = TRUE;
	    	} 
	    	else
	...
	public function rename_file()
	{
		...
		if($this->system_model->isLogged())
		{
			...
			*/
			$path = $this->system_model->userDir() . $this->input->post('rename_file_path');
			$path_parts = pathinfo($path);
			$dest_filename = $path_parts['dirname'] . '/' . $this->input->post('rename_new_name') . '.' . $path_parts['extension'];
			...
			if(file_exists($dest_filename) && $this->input->post('rename_overwrite') === 'false') {
            	...
            	{     		
            		...
            		{
            			if(rename($path, $dest . '/' . $str . $this->input->post('rename_new_name') . ' [' . $i . '].' . $extension))
            			{
            				$data = TRUE;
            			}
            			else
